...
# Function to print a list of dictionaries or lists in a tabular format
# Function to print a list of dictionaries or lists in a tabular format
def list_printer(data_list, output_file=None):
    if not data_list:
        return
...
# Function to calculate total sales, lost sales, and restock report
def generate_reports(data, lost_sales):
    total_sales_report = []
    lost_sales_report = []
    restock_report = []
    ...
    for item in data:
        ...
        # Calculate total sales and grand total
    ...
    # Generate total sales report
    total_sales_report.append(['TOTAL', '', '', '', f"${grand_total:.2f}"])

    return total_sales_report, lost_sales_report, restock_report

# Function to write reports to TXT
def export_reports_txt(total_sales_report, lost_sales_report, restock_report, output_filename):
    with open(output_filename, 'w') as f:
        list_printer(total_sales_report, f)
        f.write('\n')  # Add an empty line between reports
        list_printer(lost_sales_report, f)
        f.write('\n')
        list_printer(restock_report, f)
...
if __name__ == "__main__":
    ...
    export_reports_txt(total_sales_report, lost_sales_report, restock_report, sys.argv[2])
